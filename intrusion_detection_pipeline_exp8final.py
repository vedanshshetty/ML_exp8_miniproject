...
class IntrusionDetectionPipeline:
    def __init__(self, random_state=42, n_splits=10, n_pca=20,
...
if __name__ == "__main__":
    ...
    # Save temporary train/test splits
    df_train.to_csv("iot23_train.csv", index=False)
    df_test.to_csv("iot23_test.csv", index=False)

    # Initialize the pipeline
    pipeline = IntrusionDetectionPipeline(
        random_state=42,
        n_splits=10,           # fewer folds for faster runs
        n_pca=20,             # reduce PCA dimensions slightly (IoT-23 is smaller)
        n_clusters_kmeans=8,  # fewer clusters due to smaller data
        n_components_gmm=8,
        use_minibatch=True
    )

    # Load and run
    pipeline.load_data("iot23_train.csv", "iot23_test.csv")
    results = pipeline.run()

    # Print final model results
    if results and results['final_binary']:
        print("\n==== Final Model Results on IoT-23 ====")
